        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        <form action="{{ route('kebutuhan.update', $data->id_kebutuhan) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label class="font-weight-bold">JENIS KEBUTUHAN</label>
                                <input type="text" class="form-control @error('Jenis_kebutuhan') is-invalid @enderror" name="Jenis_kebutuhan" placeholder="Jenis Kebutuhan" value="{{ $data->Jenis_kebutuhan }}">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('keterangan') is-invalid @enderror" name="keterangan" rows="5" placeholder="Keterangan">{{ $data->keterangan }}</textarea>
                                @error('keterangan')
                                    <div class="alert alert-danger mt-2">{{ $message }}</div>
                                @enderror
                            </div>
